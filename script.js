...
const core=document.querySelector('.hero-logo-core');const emblem=document.querySelector('.hero-emblem');const dot=document.querySelector('.cursor-dot'),ring=document.querySelector('.cursor-ring');let mouseX=.5,mouseY=.5;
window.addEventListener('mousemove',e=>{mouseX=e.clientX/innerWidth;mouseY=e.clientY/innerHeight;document.documentElement.style.setProperty('--mx',(mouseX*100)+'%');document.documentElement.style.setProperty('--my',(mouseY*100)+'%');if(emblem){emblem.style.setProperty('--emblem-rx',((.5-mouseY)*7)+'deg');emblem.style.setProperty('--emblem-ry',((mouseX-.5)*10)+'deg')}if(dot){dot.style.left=e.clientX+'px';dot.style.top=e.clientY+'px'}if(ring)ring.animate({left:e.clientX+'px',top:e.clientY+'px'},{duration:180,fill:'forwards'})});document.querySelectorAll('a,button').forEach(el=>{el.addEventListener('mouseenter',()=>ring?.classList.add('hover'));el.addEventListener('mouseleave',()=>ring?.classList.remove('hover'))});
